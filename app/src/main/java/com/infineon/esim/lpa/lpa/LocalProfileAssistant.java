...
import com.infineon.esim.lpa.lpa.task.ClearNotificationsTask;
import com.infineon.esim.lpa.lpa.task.HandleAllNotificationsTask;
...
public final class LocalProfileAssistant extends LocalProfileAssistantCoreImpl implements EuiccConnectionConsumer, InternetConnectionConsumer {
    ...
    public void clearNotifications() {
        statusAndEventHandler.onStatusChange(ActionStatus.CLEAR_NOTIFICATIONS_STARTED);

        ClearNotificationsTask clearNotificationsTask = new ClearNotificationsTask(this);

        new TaskRunner().executeAsync(clearNotificationsTask,
                result -> statusAndEventHandler.onStatusChange(ActionStatus.CLEAR_NOTIFICATIONS_FINISHED),
                e -> statusAndEventHandler.onError(new Error("Error during clearing eUICC notifications.", e.getMessage())));
    }

    public void handleAllNotifications() {
        statusAndEventHandler.onStatusChange(ActionStatus.HANDLE_ALL_NOTIFICATIONS_STARTED);

        HandleAllNotificationsTask handleAllNotificationsTask = new HandleAllNotificationsTask(this);

        new TaskRunner().executeAsync(handleAllNotificationsTask,
                result -> statusAndEventHandler.onStatusChange(ActionStatus.HANDLE_ALL_NOTIFICATIONS_FINISHED),
                e -> statusAndEventHandler.onError(new Error("Error during handle all eUICC notifications.", e.getMessage())));
    }
