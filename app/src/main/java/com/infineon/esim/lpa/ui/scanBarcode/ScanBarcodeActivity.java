...
final public class ScanBarcodeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.viewModel.getActionStatus().observe(this, actionStatusObserver);

        // Attach the UI
        attachUi();

        // Refreshing eUICC info
        this.viewModel.refreshEuiccInfo();
    }
        ...
        textViewBarCodeValue.post(() -> {
            ...
            if (activationCode.isValid()) {
                if (buttonUseThisCode != null) {
                    ...
                    buttonUseThisCode.setTextColor(getColor(R.color.colorRed));
    ...
    private static String parseMemoryData(String hexData) {
        String str = hexData;
        int tagIndex = str.indexOf("82");
        if (tagIndex != -1) {
            ...
            return String.format(Locale.US,"%.2f",dataKB);
        } else return "unknow";
    }
    final Observer<AsyncActionStatus> actionStatusObserver = actionStatus -> {
        ...
        switch (actionStatus.getActionStatus()) {
            ...
            case GETTING_EUICC_INFO_FINISHED:
                textBody.setText(getString(R.string.scan_qr_add_profile_with_qr_body,
                    parseMemoryData(viewModel.getEuiccInfo().getExtCardResource())));
                break;
            default:
                // nothing
        }
