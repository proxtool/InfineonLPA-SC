...
public class DataModel implements StatusAndEventHandler{
    ...
    public void switchEuicc() {
        List<String> euiccNames = getEuiccListLiveData().getValue();
        String currentEuicc = getCurrentEuiccLiveData().getValue();
        int index = euiccNames.indexOf(currentEuicc);
        if (index != -1) {
            String euiccName = euiccNames.get((index+1)%euiccNames.size());
            Log.debug(TAG, "Switching euicc " + euiccName + "...");
            euiccManager.selectEuicc(euiccName);
        }        
    }
    ...
    public void clearNotifications() {
        lpa.clearNotifications();
    }

    public void handleAllNotifications() {
        lpa.handleAllNotifications();
    }
